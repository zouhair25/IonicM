...
export class SearchJaunePage implements OnInit{
  ...
  ionViewDidEnter (){
       if(this.quiquoi && (this.ou=='Autour de moi' || this.ou=='autour de moi')){
          this.list=this.go_build_quiquoi_approxy(this.quiquoi,this.currentLat,this.currentLng);
        ...
        console.log('lng',this.currentLng);
          ...
          console.log('je ss in normalr:', this.ou);
       }    
  }
  ...
  ngOnInit(){
      ...
      listesResultats(quiqoui,ou){
       ...
       go_build_quiquoi_approxy(quiquoi,lat,lng){
       this.go_search_quiquoi_approxy(quiquoi,lat,lng).then((data)=>{
   ...
   go_search_quiquoi_approxy(quiquoi, lat, lng){
         let list: any = [];
         let count;
         let i;
